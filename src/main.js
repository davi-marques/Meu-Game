...
Width = canvas.width = 1365; // 1152 - 1365
Height = canvas.height = 746; // 550 - 753
...
class Man {
    ...
    joiaPega(){
        if (this.pos.x+21 <= joia.pos.x + joia.width && this.pos.x-21 + this.width >= joia.pos.x && this.pos.y + this.height >= joia.pos.y && this.pos.y <= joia.pos.y + joia.height){
            ...
            // this.speed.x = 0;
            // if (this.pos.y + this.height >= joia.pos.y + joia.height+20) {
            //     this.speed.y = 0;
            // }
...
(function update() {
    ...
    if (estadoAtual == state.perdeu) {
        ...
        ctx.fillText('Você morreu!', Width/2-190, Height/2-170);
        
        ctx.font = "25px 'Press Start 2P', Arial";
        ctx.fillText('pres. Enter para reiniciar', Width/2-300, Height/2-30)
    } else if (estadoAtual == state.ganhou) {
        ctx.fillStyle = 'rgba(0, 0, 10, 0.40)'
        ctx.fillRect(0, 0, Width, Height);

        ctx.fillStyle = '#fff'
        ctx.font = "35px 'Press Start 2P', Arial";
        ctx.fillText('Você ganhou!', Width/2-190, Height/2-170);
    ...
    switch (key) {
        ...
        case 'Enter':
            if (estadoAtual == state.perdeu) {
                ...
                location.assign("file://C:/Users/davim/OneDrive/Documentos/Meus-Projetos/Meu-Game/src/fases/fase1/index.html");
            }
            break;
    }
}, false);
